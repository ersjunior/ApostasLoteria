...
from app.ui.theme import page_title, section, metric_card
...
page_title(
    ...
    "Análise histórica e estatística dos sorteios"
)
...
st.markdown(
    "Cálculo baseado em **combinações matemáticas reais**, "
    "não em frequência histórica."
)

col_input, col_output = st.columns([1, 1])

with col_input:
    qtd_apostas = st.slider(
        "🎟️ Quantidade de apostas",
        min_value=1,
        max_value=100,
        value=1
    )
    ...
    if config["total_bolas"] >= config["universo"]:
        ...
        st.info(
            f"🎯 Nesta loteria, a quantidade de dezenas é fixa: "
            f"**{qtd_dezenas} dezenas**."
        )
    else:
...
with col_output:
    ...
    with c1:
        metric_card(
            "Probabilidade por aposta",
            f"{prob_por_aposta:.10%}",
            "🎯"
        )
        st.markdown("<div style='margin-top:20px'></div>", unsafe_allow_html=True)
        metric_card(
...
with col_left:
    ...
    fig_top.update_layout(
        title=f"🔥 Top {top_n} Mais Sorteadas — {lottery_name}",
        height=400
    )
...
with col_right:
    fig_bottom = go.Figure(
        data=[go.Pie(
            labels=bottom.index.astype(str),
            values=bottom.values,
            hole=0.5
        )]
    )
    fig_bottom.update_layout(
        title=f"❄️ Top {top_n} Menos Sorteadas — {lottery_name}",
        height=400
    )
...
st.dataframe(
    prob_df.style.format({"Probabilidade": "{:.4%}"}),

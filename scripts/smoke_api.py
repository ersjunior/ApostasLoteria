...
def check_health(client: httpx.Client, base: str) -> None:
    ...
    print(
        f"OK  GET /health → status=ok (database.exists={payload.get('database', {}).get('exists')})"
    )
...
def check_lotteries(client: httpx.Client, base: str) -> None:
    ...
    payload = response.json()
    items = (
        payload if isinstance(payload, list) else payload.get("lotteries") or payload.get("items")
    )
